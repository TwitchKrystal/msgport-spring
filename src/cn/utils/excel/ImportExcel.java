...
public class ImportExcel {
	...
	/**
	 * Excel公式 解析器  add fyf 2016/12/8
	 */
	private FormulaEvaluator evaluator;
	/**
	 * Excel文件的保存位置  add by liangcq 2018-1-17
	 */
	...
	public ImportExcel(MultipartFile multipartFile, int headerNum, int sheetIndex) 
		...
		// 保存导入的excel, 20170417 从每个类中的方法转移到公共方法中
		this.setFilePath(FileUtils.saveImportExcelToDisk(multipartFile));
	}
		...
		try{
			...
			if (cell != null){
				if (cell.getCellType() == Cell.CELL_TYPE_NUMERIC){
					//cell.getNumericCellValue();  before
					//modify by fuyf 2016/12/08 start
					if(DateUtil.isCellDateFormatted(cell)){
						val=cell.getNumericCellValue();
					}else{
						val = df.format(cell.getNumericCellValue());
					}
					//modify by fuyf 2016/12/08 end
					...
					CellValue cellValue = evaluator.evaluate(cell);//解析公式
						...
						if (valType == String.class){
							...
							try{
								...
								try{
									...
									val = DateUtil.parseYYYYMMDDDate( val.toString());
								}
							}
						}else{
